  ...
  useEffect(() => {
    const fetchForms = async () => {
      ...
      try {
        ...
        // fetch full details (fields)
        const detailsRes = await fetch(`${API_BASE_URL}/forms/${active.id}`, { headers: { 'x-sme-id': user.id } });
        const detailsJson = await detailsRes.json();
        if (detailsJson.success && detailsJson.schema) {
          setActiveForm(detailsJson.schema);
        } else {
          setActiveForm(active);
        }
      } catch (err) {
        // ignore - fail silently, dashboard still works
        ...
        <Tabs.List>
          {STATUS_SEQUENCE.map((status) => (
            <Tabs.Tab
              ...
              rightSection={<Badge>{ordersByStatus[status as Order['status']]?.length || 0}</Badge>}
            >
              {status}
            </Tabs.Tab>
          ))}
        ...
        {isFormLoading ? (
          ...
          <FormRenderer
            ...
            onSubmit={async (formData: Record<string, any>) => {
              ...
              try {
                ...
                // Map common fields expected by orders table
                const payload: Record<string, any> = {
                  sme_id: smeId,
                  readable_id: readableId,
                  status: 'NEW',
                  form_data: formData, // Store complete form response as JSON
                };

                // Extract and copy known fields for backwards compatibility
                ...
                if (formData.delivery_address) payload.delivery_address = formData.delivery_address;
                if (formData.price_total) payload.price_total = parseFloat(formData.price_total) || 0;
                ...
                const messageText = generateOrderStatusMessage('NEW', readableId);
                const result = await sendWhatsAppMessage({ phone: payload.customer_phone, message: messageText, orderId: readableId });
                if (!result.success) setWhatsappError(`Order created but WhatsApp notification failed: ${result.error}`);
          ...
          <Stack gap="lg">
            ...
            <NumberInput
              ...
              onChange={(val) => setNewOrderData({ ...newOrderData, price_total: typeof val === 'number' ? val : 0 })}
            />

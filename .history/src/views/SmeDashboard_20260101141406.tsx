...
const SmeDashboard: React.FC = () => {
  ...
  const handleCreateOrder = async (formDataParam?: Record<string, any>) => {
    setIsCreating(true);
    const readableId = `WOT${Date.now().toString().slice(-6)}`;
    
    // Use form data from FormRenderer or fallback to newOrderData for manual form
    const orderData = formDataParam || newOrderData;
    
    // Extract known fields from form data
    const extractedData = {
      customer_name: orderData.customer_name || '',
      customer_phone: orderData.customer_phone || '',
      delivery_address: orderData.delivery_address || '',
      price_total: orderData.price_total ? Number(orderData.price_total) : 0,
      form_data: formDataParam ? orderData : null, // Store complete form response if from FormRenderer
    };
  ...
  return (
    <Container size="xl" py="xl">
      ...
      <Modal opened={isOrderModalOpen} onClose={() => setIsOrderModalOpen(false)} title={<Text fw={900}>New Delivery Request</Text>} centered radius="lg" size="lg">
        {activeForm ? (
          <Box p="md"><FormRenderer schema={activeForm} isLoading={isCreating} onSubmit={(formData) => handleCreateOrder(formData)} /></Box>
        ) : (
          <Stack gap="md" p="md">

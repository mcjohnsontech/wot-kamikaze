...
router.post('/csv-mapper', async (req: Request, res: Response) => {
  ...
  for (const formField of Object.values(columnMapping)) {
    ...
    const csvKey = Object.keys(columnMapping).find(key => columnMapping[key] === formField);
  ...
  if (insertError) {
    ...
    return res.status(500).json({ success: false, error: 'Failed to bulk import records' });
  }

...
import { Upload, ChevronDown, Loader } from 'lucide-react';
...
error: (error) => {
    ...
    reader.onload = async (event) => {
      ...
      // Auto-detect column mappings
      try {
        const response = await fetch(`${API_BASE_URL}/csv-mapper/auto-detect`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-sme-id': smeId,
          },
          body: JSON.stringify({ csvData: content, schemaId }),
        });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 rounded-2xl p-6 text-white">
        ...
        <p className="text-purple-100">Bulk import order data from spreadsheets</p>
      </div>
      ...
      {step === 'mapping' && csvHeaders.length > 0 && (
        <div className="space-y-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
            <h2 className="text-xl font-bold text-white mb-4">📋 Map CSV Columns to Form Fields</h2>
      ...
      {step === 'preview' && previewData.length > 0 && (
        <div className="space-y-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
            <h2 className="text-xl font-bold text-white mb-4">👁️ Preview Data ({previewData.length} rows)</h2>
                ...
                <thead className="border-b border-slate-600">
                  <tr>
                    {Object.keys(previewData[0] || {}).map((key) => (
                      <th key={key} className="text-left px-3 py-2 text-slate-300 font-semibold">
                        {key}
                      </th>
                    ))}
                  </tr>

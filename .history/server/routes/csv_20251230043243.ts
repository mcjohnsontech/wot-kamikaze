  ...
  try {
    ...
    if (importMode === "import") {
      ...
      const dataToInsert = mappedRows.map((mappedData) => ({
        schema_id: schemaId,
        sme_id: smeId,
        // order_id: order_id || null, // Ensure it's null if not provided
        order_id: null, // No order association during CSV import
        response_data: mappedData, // The rest goes into the JSON block
      }));
